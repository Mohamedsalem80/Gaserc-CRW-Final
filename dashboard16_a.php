...
// Initialize the array for each subject and grade
...
foreach ($subjectIds as $subjectId => $subjectName) {
    $teachingHoursQuery = "
        SELECT th.GradeID, SUM(th.AnnualHours) AS TotalAnnualHours
    ...
    if ($teachingHoursResult->num_rows > 0) {
        while ($teachingHoursRow = $teachingHoursResult->fetch_assoc()) {
            ...
            $totalAnnualHours = $teachingHoursRow['TotalAnnualHours'];

            // Store the total hours for each grade
            if (isset($countryDatasets[$subjectId][$gradeId])) {
                $countryDatasets[$subjectId][$gradeId] = $totalAnnualHours;
            }
        }
    }
}
...
foreach ($countryDatasets as $subjectId => $data) {
    ...
    foreach ($data as $grade => $hours) {
        if ($grade !== 'subject_name') { // Skip the subject name key
            ...
            // Assign the total annual hours for each subject and grade
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="addSubForm" action="addsubject.php" method="POST">
                                ...
                                <div class="form-group">
                                    <label class="col-form-label">الاوزان النسبية للمواد:</label>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <input type="text" class="form-control" name="minHours" placeholder="الحد الادنى">
                                        </div>
                                        <div class="col-md-4">
                                        <input type="text" class="form-control" name="maxHours" placeholder="الحد الاعلى">
                                        </div>
                                        <div class="col-md-4">
                            ...
                            <div class="container mt-2">
                                <div class="form-group row mb-1">
                                    ...
                                    <?php
                                        if (isset($_SESSION['job']) && $_SESSION['job'] != 1) {
                                    ?>
                                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal" data-whatever="@mdo" style="width: 150px;">اضف مادة</button>
                                    <?php
                                        }
                                    ?>
                                    </div>
                ...
                <div class="row" style="margin-top: 40px;">
                    <div class="col-md-12">
                        ...
                        <div class="btn-group d-flex justify-content-center" role="group" style="margin-top: 4px;">
                            <button class="btn btn-primary custom-btn mx-1" onclick="updateCountryPieChart('Islamic Education')">التربية الإسلامية</button>
                            <button class="btn btn-primary custom-btn mx-1" onclick="updateCountryPieChart('Arabic Language')">اللغة العربية</button>
                            <button class="btn btn-primary custom-btn mx-1" onclick="updateCountryPieChart('English Language')">اللغة الإنجليزية</button>
                            <button class="btn btn-primary custom-btn mx-1" onclick="updateCountryPieChart('Mathematics')">الرياضيات</button>
                            <button class="btn btn-primary custom-btn mx-1" onclick="updateCountryPieChart('Science')">العلوم</button>
                            <button class="btn btn-primary custom-btn mx-1" onclick="updateCountryPieChart('Social Studies')">الدراسات الاجتماعية</button>
                            <button class="btn btn-primary custom-btn mx-1" onclick="updateCountryPieChart('Physical Education')">التربية البدنية</button>
                        </div>
                        <div class="btn-group d-flex justify-content-center" role="group" style="margin-top: 4px;">
